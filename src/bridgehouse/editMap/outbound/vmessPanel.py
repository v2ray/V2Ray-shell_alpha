...
class OutboundVmessSettingPanel(QWidget):
    ...
    def onbtnoutVmessChange(self):
        if (self.treeViewOutboundVmessAddressMode.rowCount() == 0): return
    ...
    def onbtnoutVmessAdd(self):
        if (self.treeViewOutboundVmessAddressMode.rowCount() == 0):
            if (self.lineEditOutboundVmessAddress.text() == ""): return
        ...
        if (root == -1): ### user clicked address
            ...
            if ((self.treeViewOutboundVmessAddressMode.item(row).hasChildren() == False) and
    ...
    def settingOutboundVmessPanelFromJSONFile(self, outboundVmessJSONFile = {}, openFromJSONFile = False):
        logbook.setisOpenJSONFile(openFromJSONFile)
        self.treeViewOutboundVmessAddressMode.setRowCount(0)
        
        if (outboundVmessJSONFile == None): outboundVmessJSONFile = {}
    ...
    def createOutboundVmessJSONFile(self):
        ...
        if (serversNumber < 0): return outboundVmessJSONFile
